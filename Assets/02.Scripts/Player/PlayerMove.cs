...
public class PlayerMove : MonoBehaviour
{
    ...
    public float obstacleSensingDistance = 3f; // 장애물 감지 거리
    public LayerMask obstacleLayer; // 장애물 레이어
    private bool obstacleDetected = false; // 장애물 감지 여부
    ...
    void Update()
    {
        ...
        if (GameManager.Instance.MoveMode == 0)
        {
            ...
            if (currentSpeed > 0)
            {
                ...
                // 장애물 감지 및 처리
                obstacleDetected = Physics.Raycast(transform.position, moveDirection, obstacleSensingDistance, obstacleLayer);
        ...
        else if (GameManager.Instance.MoveMode == 1)
        {
            Vector2 input = continuousMoveAction.action.ReadValue<Vector2>();
            Vector3 desiredMove = ComputeDesiredMove(input);

            // MoveMode 0과 동일하게 가속 및 감속 적용
            ...
            {
                // 장애물 감지 및 처리
                obstacleDetected = Physics.Raycast(transform.position, desiredMove.normalized, obstacleSensingDistance, obstacleLayer);
    ...
    Vector3 ComputeDesiredMove(Vector2 input)
    {
        ...
        var speedFactor = currentSpeed * Time.deltaTime;

        return moveDirection * speedFactor;
    }
}