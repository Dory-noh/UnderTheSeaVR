    ...
    void FishSpawn(int i, int j)
    {
        ...
        foreach (var fish in fishList[i+j])
        {
            ...
            if (fish.activeSelf == false)
            {
                ...
                fish.transform.position = new Vector3(Random.Range(RespawnPoint.position.x - 50, RespawnPoint.position.x + 50), Random.Range(RespawnPoint.position.y - 30, RespawnPoint.position.y + 30), Random.Range(RespawnPoint.position.z - 50, RespawnPoint.position.z + 50));
